...
<div style="max-width: 900px; margin: 0 auto; position: relative;">
    ...
    <div class="glass-card" style="padding: 1.25rem 1.75rem; margin-bottom: 1.5rem; display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 1rem; border: 1px solid rgba(255,255,255,0.12);">
        
        <!-- Round & Difficulty Badge -->
        <div style="display: flex; align-items: center; gap: 0.75rem;">
            <div id="round-badge" style="padding: 0.4rem 0.85rem; border-radius: 9999px; font-weight: 800; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px; background: rgba(16, 185, 129, 0.2); border: 1px solid rgba(16, 185, 129, 0.4); color: #34d399;">
                <i class="fa-solid fa-layer-group"></i> <span id="round-text">ROUND 1 &bull; EASY</span>
            </div>
            
            <div id="category-badge" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); padding: 0.4rem 0.85rem; border-radius: 9999px; font-size: 0.85rem; color: #cbd5e1; font-weight: 600; max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
            <!-- Streak Flame -->
            <div id="streak-container" style="display: flex; align-items: center; gap: 0.4rem; background: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.35); padding: 0.4rem 0.85rem; border-radius: 9999px; font-weight: 800; font-size: 0.9rem; color: #fbbf24; transition: transform 0.2s ease;">
                <i class="fa-solid fa-fire text-amber-400" id="streak-icon"></i>
                <span id="streak-count">0</span> STREAK
            </div>

            <!-- Live Net Delta -->
            <div style="display: flex; align-items: center; gap: 0.4rem; background: rgba(99, 102, 241, 0.15); border: 1px solid rgba(99, 102, 241, 0.3); padding: 0.4rem 0.85rem; border-radius: 9999px; font-weight: 800; font-size: 0.9rem; color: #a5b4fc;">
    ...
    <div style="margin-bottom: 1.5rem;">
        <div style="display: flex; justify-content: space-between; font-size: 0.85rem; font-weight: 700; color: #94a3b8; margin-bottom: 0.5rem;">
        ...
        <div style="width: 100%; height: 8px; background: rgba(255,255,255,0.08); border-radius: 9999px; overflow: hidden; position: relative;">
            <div id="progress-bar-fill" style="height: 100%; width: 3.33%; background: linear-gradient(90deg, #6366f1, #06b6d4); border-radius: 9999px; transition: width 0.4s ease;"></div>
        </div>
    </div>

    <!-- Main Question Arena Card -->
    <div class="glass-card" style="padding: 2.5rem; position: relative; border: 1px solid rgba(99, 102, 241, 0.3); box-shadow: 0 16px 48px rgba(0,0,0,0.5);">
        ...
        <div style="display: flex; justify-content: center; margin-bottom: 1.75rem;">
            <div style="position: relative; width: 84px; height: 84px; display: flex; align-items: center; justify-content: center;">
                <svg width="84" height="84" viewBox="0 0 84 84" style="transform: rotate(-90deg);">
                    <circle cx="42" cy="42" r="36" stroke="rgba(255,255,255,0.1)" stroke-width="6" fill="transparent" />
                    <circle id="timer-circle" cx="42" cy="42" r="36" stroke="#10b981" stroke-width="6" fill="transparent" stroke-dasharray="226" stroke-dashoffset="0" stroke-linecap="round" style="transition: stroke-dashoffset 0.1s linear, stroke 0.3s ease;" />
                </svg>
                <div id="timer-number" style="position: absolute; font-size: 1.75rem; font-weight: 900; color: #fff; font-family: 'Space Grotesk', sans-serif;">
                    5
                </div>
            </div>
        </div>
        ...
        <!-- Question Prompt -->
        <div style="min-height: 90px; display: flex; align-items: center; justify-content: center; text-align: center; margin-bottom: 2rem;">
            <h2 id="question-text" style="font-size: 1.35rem; font-weight: 700; color: #fff; line-height: 1.45;">
                Fetching question from OpenTDB...
            </h2>
        </div>

        <!-- Streak Bonus Announcement Pop (Hidden by default) -->
        <div id="combo-pop" style="text-align: center; height: 28px; margin-bottom: 1rem; opacity: 0; transition: all 0.3s ease;">
            <span id="combo-text" style="background: linear-gradient(135deg, #f59e0b, #ef4444); color: #fff; font-weight: 800; font-size: 0.9rem; padding: 0.3rem 0.85rem; border-radius: 9999px; box-shadow: 0 0 16px rgba(245, 158, 11, 0.5);">
                🔥 3x COMBO BONUS (+1 PTS)!
            </span>
        </div>

        <!-- 4 Choices Grid -->
        <div id="choices-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1rem;">
            <!-- Choice buttons will be dynamically rendered here -->
        </div>

        <!-- Feedback Result Message under choices -->
        <div id="feedback-container" style="margin-top: 1.5rem; text-align: center; min-height: 32px; font-weight: 700; font-size: 1.05rem; opacity: 0; transition: opacity 0.2s ease;">
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 1.25rem;">
        <form action="{{ route('game.abandon', ['sessionId' => $session->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to forfeit this match? Points earned or lost will be finalized.')">
            @csrf
            <button type="submit" class="btn btn-outline" style="color: #94a3b8; font-size: 0.85rem; padding: 0.45rem 0.85rem;">
...
<div id="roundBreakModal" class="modal-overlay">
    <div class="modal-card" style="text-align: center; max-width: 520px; border: 1px solid rgba(99, 102, 241, 0.3);">
        <div style="width: 64px; height: 64px; margin: 0 auto 1.25rem; background: linear-gradient(135deg, #6366f1, #06b6d4); border-radius: 20px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 2rem; box-shadow: 0 0 24px rgba(99, 102, 241, 0.5);">
            <i class="fa-solid fa-forward-step"></i>
        </div>

        <h2 id="break-title" style="font-size: 1.75rem; font-weight: 900; color: #fff; margin-bottom: 0.5rem;">Round 1 Complete!</h2>
        ...
        <!-- Round Stats Card -->
        <div style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 1rem; padding: 1.25rem; margin-bottom: 1.5rem; display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem;">
            <div>
        ...
        <div id="next-round-banner" style="background: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.35); border-radius: 0.85rem; padding: 0.85rem; margin-bottom: 1.5rem; text-align: left; font-size: 0.85rem; color: #fef3c7;">
        ...
        <button type="button" class="btn btn-primary" style="width: 100%; padding: 0.9rem; font-size: 1.1rem; font-weight: 700; border-radius: 1rem;" onclick="proceedToNextRound()">
...
<div id="bankruptcyModal" class="modal-overlay">
    <div class="modal-card" style="text-align: center; max-width: 480px; border: 2px solid #ef4444; box-shadow: 0 0 40px rgba(239, 68, 68, 0.4);">
        <div style="width: 68px; height: 68px; margin: 0 auto 1.25rem; background: rgba(239, 68, 68, 0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #ef4444; font-size: 2rem; animation: pulse 1.5s infinite;">
            <i class="fa-solid fa-triangle-exclamation"></i>
        </div>

        <h2 style="font-size: 1.6rem; font-weight: 900; color: #fff; margin-bottom: 0.4rem;">You Ran Out of Points!</h2>
        <p style="color: #fca5a5; font-size: 0.95rem; margin-bottom: 1.5rem; line-height: 1.4;">
            You cannot continue playing because your points balance has reached <strong>0 or below</strong>. Please add points to proceed with your match!
        </p>

        <!-- Quick selection -->
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.6rem; margin-bottom: 1.25rem;">
            <button type="button" class="btn btn-outline" style="border-color: rgba(239, 68, 68, 0.4); font-size: 0.9rem; font-weight: 700; color: #fca5a5;" onclick="setBankruptTopUp(100)">
                +100 PTS
            </button>
            <button type="button" class="btn btn-outline" style="border-color: rgba(239, 68, 68, 0.4); font-size: 0.9rem; font-weight: 700; color: #fca5a5;" onclick="setBankruptTopUp(200)">
                +200 PTS
            </button>
            <button type="button" class="btn btn-outline" style="border-color: rgba(239, 68, 68, 0.4); font-size: 0.9rem; font-weight: 700; color: #fca5a5;" onclick="setBankruptTopUp(500)">
                +500 PTS
            </button>
        </div>

        <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-size: 0.85rem; font-weight: 600; color: #cbd5e1; margin-bottom: 0.4rem; text-align: left;">
                Enter Points to Add:
            </label>
            <input type="number" id="bankruptPointsInput" value="100" min="20" max="10000" class="form-input" style="border-color: rgba(239, 68, 68, 0.5); font-size: 1.1rem; font-weight: 700; color: #fbbf24; text-align: center;">
        </div>

        <button type="button" id="bankruptConfirmBtn" class="btn btn-gold" style="width: 100%; padding: 0.9rem; font-size: 1.05rem;" onclick="submitBankruptTopUp()">
...
<div id="antiCheatModal" class="modal-overlay">
    <div class="modal-card" style="text-align: center; max-width: 480px; border: 2px solid #ef4444; box-shadow: 0 0 50px rgba(239, 68, 68, 0.5);">
        <div style="width: 72px; height: 72px; margin: 0 auto 1.25rem; background: rgba(239, 68, 68, 0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #ef4444; font-size: 2.25rem; animation: pulse 1.2s infinite;">
            <i class="fa-solid fa-shield-virus"></i>
        </div>

        <h2 style="font-size: 1.65rem; font-weight: 900; color: #fff; margin-bottom: 0.5rem;">Anti-Cheat Violation!</h2>
        <p style="color: #fca5a5; font-size: 0.95rem; margin-bottom: 1.5rem; line-height: 1.5;">
            Tab switching or leaving the game window was detected. To prevent cheating, your match has been <strong>automatically cancelled</strong> and entry points forfeited.
        </p>

        <div style="background: rgba(255,255,255,0.05); border-radius: 0.85rem; padding: 0.85rem; font-size: 0.85rem; color: #cbd5e1; margin-bottom: 1.5rem;">
            Redirecting to Home Hub in <strong id="antiCheatCountdown" style="color: #fbbf24;">2</strong> seconds...
        </div>

        <a href="{{ route('user.home') }}" class="btn btn-danger" style="width: 100%; padding: 0.85rem; font-size: 1rem;">
...
<script>
    ...
    let timeLeft = 5.0; // 5-second interval
    const totalTime = 5.0;
    ...
    async function submitAnswer(userChoice) {
        try {
            const res = await fetch(`/game/${sessionId}/answer`, {
                ...
                body: JSON.stringify({ answer: userChoice })
            });
            ...
            if (comboPop && comboText) {
                if (result.is_correct && result.streak_bonus > 0) {
                    comboText.textContent = `🔥 ${result.current_streak}x STREAK COMBO (+${result.streak_bonus} BONUS PTS)!`;
            ...
            if (fbText) {
                if (result.is_correct) {
                    fbText.innerHTML = `<span style="color: #34d399;"><i class="fa-solid fa-circle-check"></i> CORRECT! +${result.points_awarded} PTS</span>`;
                } else {
                    if (userChoice === null) {
                        fbText.innerHTML = `<span style="color: #fb7185;"><i class="fa-solid fa-clock"></i> TIMED OUT! ${result.points_awarded} PTS (Correct answer: ${result.correct_answer})</span>`;
                    } else {
                        fbText.innerHTML = `<span style="color: #fb7185;"><i class="fa-solid fa-circle-xmark"></i> WRONG! ${result.points_awarded} PTS (Correct answer: ${result.correct_answer})</span>`;
                    }
                }
            }
            ...
            const pointsDeltaEl = document.getElementById('points-delta');
            if (pointsDeltaEl) {
                pointsDeltaEl.textContent = result.points_delta >= 0 ? '+' + result.points_delta : result.points_delta;
            ...
            // Short 650ms pause for instant feedback animation, then immediately load next question
            setTimeout(() => {
    ...
    function showRoundBreakModal(result) {
        ...
        if (result.next_round === 2) {
            ...
            banner.innerHTML = `<strong>Next: Round 2 (Questions 11–20)</strong> &bull; Medium difficulty &bull; <span style="color: #34d399;">+3 PTS</span> / <span style="color: #fb7185;">-5 PTS</span>`;
        } else if (result.next_round === 3) {
            title.textContent = 'Round 2 (Normal) Completed! 🔥';
            banner.innerHTML = `<strong>Next: Round 3 (Questions 21–30)</strong> &bull; Hard difficulty &bull; <span style="color: #34d399;">+5 PTS</span> / <span style="color: #fb7185;">-10 PTS</span>`;
        }

        document.getElementById('break-correct').textContent = `${result.next_question_index - 1 - (result.is_correct ? 0 : 1)} Qs`;
        document.getElementById('break-streak').textContent = `${result.current_streak} 🔥`;
        document.getElementById('break-points').textContent = `${result.points_delta >= 0 ? '+' : ''}${result.points_delta} PTS`;
    ...
    window.addEventListener('beforeunload', (e) => {
        if (!hasDisqualified && !isGameCompletedState) {
            e.preventDefault();
            e.returnValue = 'Warning: Leaving or reloading the page will forfeit your match and cannot be resumed!';
            try {
